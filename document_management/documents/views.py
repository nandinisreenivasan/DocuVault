...
def upload_document(request):
    ...
    if not email or not auth_token:
        return Response({"error": "Email and Authorization headers are required"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        user = User.objects.get(email=email)
        if user.id != user_id:
            return Response({"error": "Email does not match the token's user."}, status=status.HTTP_401_UNAUTHORIZED)
    ...
    if not text or not pages:
        return Response({"error": "Text and pages are required."}, status=status.HTTP_400_BAD_REQUEST)
...
def list_documents(request):
    ...
    try:
        ...
        page_number = int(request.query_params.get('page', settings.DEFAULT_PAGE_NUMBER))
